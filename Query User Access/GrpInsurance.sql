...
SELECT
	...
	group_menu.T_Print
FROM
	dbo.gUser_Name AS group_user
	LEFT JOIN
	dbo.gMenu_Group AS group_menu
	ON group_user.id_user = group_menu.id_user
	LEFT JOIN
	dbo.gMenu_Item AS menu_item
...
SELECT
	...
	[user_name].Change_Date AS change_date,
	[group_user].tingkat,
	dept.Deskripsi
FROM
	dbo.gUser_Name AS [user_name]
	LEFT JOIN
	dbo.gGroup_User AS group_user
	ON [user_name].Id_User = group_user.Id_User2
	LEFT JOIN
	dbo.gUser_Dept AS dept
	ON dept.Id_Dept = [user_name].Id_Dept
WHERE
	[user_name].tipe = 1 AND
	[user_name].Status = 'I' --AND
	--[user_name].Id_User IN
	----(
	---- 'BAI'
	----,'IYT'
	----,'MSR'
...
SELECT
	...
	group_user.change_date AS ChangeDate,
	group_user.tingkat,
